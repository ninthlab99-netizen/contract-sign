...
(function () {
  ...
  // 之前，內部自動補上固定的前後綴，讓實際送出的密碼一定符合長度規定，
  // 使用者畫面上輸入的內容完全不受影響。
  ...
  function attemptLogin() {
    ...
    // 注意：新版 Firebase Auth 對「帳號不存在」跟「密碼錯誤」會回傳同一種
    // 泛用錯誤代碼（auth/invalid-credential），沒辦法用錯誤代碼分辨兩者。
    // 所以改成「先嘗試建立帳號」：
    //   - 如果帳號還不存在（第一次使用）→ 建立成功，直接登入。
    //   - 如果帳號已存在（auth/email-already-in-use）→ 改用登入方式驗證，
    //     這樣密碼打錯時，一定會從「登入」這一步驟收到明確的密碼錯誤代碼，
    //     不會再誤判成帳號不存在而卡住。
    fsAuth.createUserWithEmailAndPassword(ADMIN_EMAIL, realPw).catch(function (err) {
      if (err && err.code === 'auth/email-already-in-use') {
        return fsAuth.signInWithEmailAndPassword(ADMIN_EMAIL, realPw);
      }
      throw err;
    }).catch(function (err) {
      ...
      if (code === 'auth/wrong-password' || code === 'auth/invalid-credential' || code === 'auth/invalid-login-credentials' || code === 'auth/user-not-found') {
        ...
        msg = '此網址尚未被授權登入，請聯絡技術人員將此網域加入 Firebase 授權清單。';
      } else if (code === 'auth/too-many-requests') {
        msg = '嘗試次數過多，請稍候幾分鐘後再試一次。';
      } else if (code) {
        msg = '登入發生錯誤（' + code + '），請截圖回報。';
      }
  ...
  function loadContractList() {
    ...
    fsDB.collection('contracts').orderBy('createdAt', 'desc').get().then(function (snap) {
      ...
      if (snap.empty) {
        contractList.innerHTML = '<p class="muted">目前沒有任何合約。</p>';
        return;
      }
      var docs = snap.docs;
      ...
      Promise.all(sigChecks).then(function (sigSnaps) {
        ...
        contractList.querySelectorAll('.copy-link-item').forEach(function (btn) {
          btn.addEventListener('click', function () {
            ...
            btn.textContent = '已複製！';
            setTimeout(function () { btn.textContent = '複製簽署連結'; }, 1500);
          });
        });
      });
    }).catch(function (err) {
